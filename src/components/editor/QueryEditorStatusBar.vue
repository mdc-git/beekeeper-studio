<template>
  <statusbar :class="{ empty: results.length === 0, 'query-meta': true }">
    <template v-if="results.length > 0">
      ...
      <div
        class="statusbar-item row-counts row flex-middle"
        v-if="rowCount > 0"
        :title="rowCount + ' ' + 'Total Records'"
      >
        <i class="material-icons">list_alt</i>
        <span class="num-rows">{{ rowCount }}</span>
        <span class="truncated-rows" v-if="result && result.truncated"
          >/&nbsp;{{ result.truncatedRowCount }}</span
        >
      </div>
      <div
      ...
      <span
        class="statusbar-item execute-time row flex-middle"
    ...
    <x-buttons class="download-results" v-if="result">
      <x-button
        ...
        v-tooltip="'Download Results (CSV)'"
